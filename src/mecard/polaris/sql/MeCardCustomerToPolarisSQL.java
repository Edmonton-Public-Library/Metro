 ...
 * become '780-555-1212'. The {@link MeCardCustomerToNativeFormat} object has the ability 
...
public class MeCardCustomerToPolarisSQL implements MeCardCustomerToNativeFormat
{
     ...
     * @param customer 
     */
    public MeCardCustomerToPolarisSQL(Customer customer)
    {
        ...
        switch (props.getProperty("conformance", "default"))
        {
            case "Polaris 6.2":
            case "Polaris 6.3":
            case "Polaris 6.4":
        ...
        MeCardDataToPolarisSQLData formattedTable = new MeCardDataToPolarisSQLData(PolarisTable.PATRONS, table);
        this.customerAccount.add(formattedTable);
        table = new HashMap<>();
        formattedTable = new MeCardDataToPolarisSQLData(PolarisTable.ADDRESSES, table);
        this.customerAccount.add(formattedTable);
        table = new HashMap<>();
        formattedTable = new MeCardDataToPolarisSQLData(PolarisTable.PATRON_ADDRESSES, table);
        this.customerAccount.add(formattedTable);
        table = new HashMap<>();
        formattedTable = new MeCardDataToPolarisSQLData(PolarisTable.PATRON_REGISTRATION, table);
        this.customerAccount.add(formattedTable);
        table = new HashMap<>();
        formattedTable = new MeCardDataToPolarisSQLData(PolarisTable.POSTAL_CODES, table);
        this.customerAccount.add(formattedTable);
...
//                customer.get(CustomerFieldTypes.SEX));
     ...
     */
    private boolean containsKey(MeCardDataToNativeData table, String key)
    {
     ...
     * @param index which can always be 0 for MeCardCustomerToPolarisSQL since
 ...
 the table will just be added to the end of the list.
    ...
    public void insertTable(MeCardDataToNativeData formattedTable, int index)
    {
        ...
        {
            this.customerAccount.add((MeCardDataToPolarisSQLData)formattedTable);
        }
        else if (index < 0)
        {
            this.customerAccount.add(0, (MeCardDataToPolarisSQLData)formattedTable);
        }
        else
        {
            this.customerAccount.add(index, (MeCardDataToPolarisSQLData)formattedTable);
        }
    }
