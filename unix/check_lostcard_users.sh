...
##################################################################
#
# What:   Checks for LOSTCARD users.
...
SUBJECT="Lost card, Me Card customer report."
# We cd into the directory and look for this file later.
TOUCH_FILE=./._lostcard_users_.txt
LAST_RUN=0
LAST_RUN_DATE=""
...
if ls *.lost >/dev/null
then
	declare -a customerFiles=(`ls *.lost`)
	lostCustomerCount=0
	...
	do
		...
		then
			...
			lostCustomerCount=$[$lostCustomerCount +1]
		fi
	done

	if [ $lostCustomerCount -gt 0 ]
	then
		echo "$lostCustomerCount lost card customer(s) found. Last check: $LAST_RUN_DATE." 
		echo "$lostCustomerCount lost card customer(s) found. Last check: $LAST_RUN_DATE." | mailx -s "$SUBJECT" "$ADDRESSES"
	fi
